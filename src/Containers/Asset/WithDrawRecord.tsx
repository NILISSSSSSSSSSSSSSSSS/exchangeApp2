...
import {fiatFormate, currencyFormate, withdrawStatus, formatDate} from '../../Tools/function'
import { withdrawRecords } from '../../Apis/finance/transaction'
...
export default class WithDrawRecord extends Component<Props, State> {
  ...
  async _getData (obj: any) {
    ...
    let res = await withdrawRecords(obj)
    ...
    return(
      <div className="custom-table" key={`${sectionID}-${rowID}`} >
          <div className="table-body col-4">
            <span>{formatDate(rowData.createTime)}</span><span>{currencyFormate(rowData.amount)}</span><span>{rowData.currencyName}</span><span>{withdrawStatus(rowData.withdrawStatus)}</span>
          </div>
      </div>
    )
  }
      ...
      <div className="withdraw-record">
        ...
        />
          ...
          <div className="app-container">
            <WingBlank><h2 style={{marginBottom: 15}}>提币记录</h2></WingBlank>
